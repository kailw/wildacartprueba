...
moduleFactura.controller('facturaPlistController', ['$scope', '$http', '$location', 'toolService', '$routeParams', 'sessionService',
    function ($scope, $http, $location, toolService, $routeParams, sessionService) {
        ...
        $scope.pdf = function (id, fecha, iva, idUsuario) {
            ...
            $http({
                ...
                $scope.dibujarHeader(id, fecha, idUsuario);
                var pagina = 1;
                doc.setFontSize(10);
        ...
        $scope.dibujarHeader = function (id, fecha, idUsuario) {
            doc.setFontSize(13);
            ...
            doc.setFontSize(13);
            doc.setFontType('normal');
            for (var i = 0; i < $scope.ajaxDataFacturas.length; i++) {
                if (idUsuario === $scope.ajaxDataFacturas[i].obj_Usuario.id) {
                    doc.text(100, 37, $scope.ajaxDataFacturas[i].obj_Usuario.nombre + " " + $scope.ajaxDataFacturas[i].obj_Usuario.ape1  + " " + $scope.ajaxDataFacturas[i].obj_Usuario.ape2);
                    doc.text(100, 43, 'DNI: ' + $scope.ajaxDataFacturas[i].obj_Usuario.dni);                   
                }
            }            
